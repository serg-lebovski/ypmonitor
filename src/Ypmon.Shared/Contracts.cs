...
public class ReportAckDto
{
    ...
    // ======================================================================
    // Синхронизация настроек «сервер → агент» (не команда!).
    //
    // Это НЕ исполняемая команда, а данные-метка: тип задания архивации по папкам.
    // Агент сравнивает со своим локальным значением и, если отличается, переписывает
    // у себя. Ничего не исполняется, никаких путей/аргументов/строк для запуска.
    // Старые агенты поле игнорируют.
    // ======================================================================

    /// <summary>Тип задания по папкам {"имя папки": "Full"|"Incremental"|"Differential"}, как настроено на сервере.</summary>
    public Dictionary<string, string> FolderBackupTypes { get; set; } = new();
}
